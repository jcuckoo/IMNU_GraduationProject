...
public class LoginAction {
	...
	public void loginUser(HttpServletRequest request,
		...
		if (StringUtil.isEmpty(userName) || StringUtil.isEmpty(password)) {
			...
			}else if (loginType.equals("youke")){
				Youke youke = new Youke();
				youke.setYoukeName(userName);
				youke.setYoukePassword(password);
				try {
					if (youkeService.queryYoukes(youke, userName, 0, 0, null, null).size() == 1) {
						rizhi.setRizhiName(userName);
						rizhi.setDate(date);
						rizhi.setDengluIp(ip);
						rizhiService.save(rizhi);
						Youke youkeLogin = (Youke)(youkeService.queryYoukes(youke, userName, 0, 0, null, null)).get(0);
						// 获取Session
						HttpSession session=request.getSession();
						session.setAttribute("youke", youkeLogin);
						// 客户端跳转
						response.sendRedirect("youkeMain.jsp");
					}else{
						request.setAttribute("error", "用户名或密码错误！");
						// 服务器跳转
						request.getRequestDispatcher("index.jsp").forward(request, response);
