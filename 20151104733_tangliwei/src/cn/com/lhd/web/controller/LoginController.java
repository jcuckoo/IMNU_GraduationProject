...
public class LoginController extends CommonController {
	...
	public ModelAndView toLogin(){
		...
		mav.addObject("pageTitle", "登录页-滨海影视网");
	...
	public ModelAndView toRegister(){
		ModelAndView mav = new ModelAndView();
		mav.addObject("pageTitle", "注册页-滨海影视网");
	...
	public JsonViewData checkLogin(){
		if (Objects.isNull(getCurrentUser())) {
			return setJsonViewData(ResultCode.NO_LOGIN, "亲，请先登录！");
		}
	 ...
	 */
	@ResponseBody
	@RequestMapping(value = "/check/vip")
	public JsonViewData checkVIP(Long id){
		Film film = iFilmService.queryById(id);
		if (!BlankUtil.isBlank(film)) {
			if (getCurrentUser().getId()!=film.getCreator().getId() 
					&& getCurrentUser().getUserType().equals(USERTYPE_C) 
					&& film.getVip()) {
				return setJsonViewData(ResultCode.FAIL, "亲，请先升级为VIP用户！");
			}
		}
		return setJsonViewData(ResultCode.SUCCESS);
	}
