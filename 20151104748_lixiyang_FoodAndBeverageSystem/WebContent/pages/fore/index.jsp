...
<body>
	...
	<div id="content-container" class="container">
		<div class="es-row-wrap container-gap" id="context-homepage">
			...
			<!-- 精彩分享begin -->
			<div class="homepage-share-nav">美食分享</div>
			<!-- 精彩分享end -->
				...
				<div class="col-sm-3">
					<div class="es-box es-tag">
						...
						<div class="es-box-body">
							<div class="tag-list">
								<c:forEach var="var" items="${menuTypeCountList }">
									<a href="${ctx }/menu/list.html?menuType.id=${var.id}">${var.name }（${var.menuCount }）</a>
								</c:forEach>
							</div>
						</div>
					</div>
					<div class="es-box es-rank">
						...
						<div class="es-box-body">
							<ul class="media-list">
								<c:forEach var="var" items="${hotMenuList }">
									<li class="media">
										<a href="${ctx }/menu/view/${var.id}.html">
											<div class="es-item">
												<c:choose>
													<c:when test="${empty var.coverImg }">
														<img alt="${var.name }" src="${ctx }/static/images/default.jpg" class="img-responsive thumb pull-left">														
													</c:when>
													<c:otherwise>
														<img alt="${var.name }" src="${ctx }/${var.coverImg}" class="img-responsive thumb pull-left">
													</c:otherwise>
												</c:choose>
												<div class="es-title" style="line-height:32px">${var.name }</div>
											</div>
										</a>
									</li>
								</c:forEach>
